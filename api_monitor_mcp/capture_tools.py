...
def capture_export_overview(
    ...
    """Export a bounded capture triage report as JSON or CSV."""
    if output_format not in {"json", "csv"}:
        raise ValueError("output_format must be json or csv")
    output = Path(output_path).expanduser()
    if not output.parent.is_dir():
...
@mcp.tool()
def capture_export_search_entries(
    file_path: str,
    output_path: str,
    query: str,
    limit: int = 100,
    ...
    if output_format == "json":
        ...
        stream = io.StringIO(newline="")
        writer = csv.writer(stream)
        writer.writerow(["entry", "offset", "encoding", "text", "entry_truncated"])
        for entry in result["entries"]:
...
@mcp.tool()
def capture_export_error_summary(
    file_path: str,
    output_path: str,
    process_index: int | None = None,
    pid: int | None = None,
    ...
    """Export saved call error summaries as JSON or CSV."""
    if output_format not in {"json", "csv"}:
        raise ValueError("output_format must be json or csv")
    output = Path(output_path).expanduser()
    if not output.parent.is_dir():
        ...
        for error in result["errors"]:
            writer.writerow(
                [
                    ...
                    error.get("hex", ""),
                    error.get("count", ""),
                    json.dumps(error.get("process_indices", []), ensure_ascii=False),
                    json.dumps(error.get("pids", []), ensure_ascii=False),
                    json.dumps(error.get("thread_ids", []), ensure_ascii=False),
                    json.dumps(error.get("first_record", {}), ensure_ascii=False),
                    json.dumps(error.get("last_record", {}), ensure_ascii=False),
                    json.dumps(error.get("apis", []), ensure_ascii=False),
                    json.dumps(error.get("context", {}), ensure_ascii=False),
                ]
            )
        content = stream.getvalue()
    encoded, digest = _write_text_export(output, content, overwrite)
...
@mcp.tool()
def capture_export_search_calls(
    file_path: str,
    output_path: str,
    query: str = "",
    output_format: str = "json",
    ...
    """Export bounded saved-call search results as JSON or CSV."""
    if output_format not in {"json", "csv"}:
        raise ValueError("output_format must be json or csv")
    output = Path(output_path).expanduser()
    if not output.parent.is_dir():
    ...
    result = capture_search_calls(
        ...
        include_record_bytes=include_record_bytes,
        pid=pid,
        query_regex=query_regex,
        thread_id=thread_id,
        error_code=error_code,
        ...
        for match in result["matches"]:
            ...
            writer.writerow(
                [
                    ...
                    context.get("duration_seconds", ""),
                    context.get("error_code", ""),
                    json.dumps(match.get("payload_matches", []), ensure_ascii=False),
                    json.dumps(match.get("api_matches", []), ensure_ascii=False),
                    json.dumps(match.get("argument_matches", []), ensure_ascii=False),
                    json.dumps(match.get("byte_matches", []), ensure_ascii=False),
...
@mcp.tool()
def capture_export_calls_around(
    file_path: str,
    output_path: str,
    process_index: int,
    record_index: int,
    ...
    """Export a bounded call-record window around one saved call as JSON or CSV."""
    if output_format not in {"json", "csv"}:
        raise ValueError("output_format must be json or csv")
    output = Path(output_path).expanduser()
    if not output.parent.is_dir():
